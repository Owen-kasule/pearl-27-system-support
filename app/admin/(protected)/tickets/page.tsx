  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Filter tickets by status">{filters.map((filter) => { const query = new URLSearchParams(); if (filter.value !== "ALL") query.set("status", filter.value); if (search) query.set("q", search); return <Link key={filter.value} href={`/admin/tickets?${query}`} className={`focus-ring min-h-10 shrink-0 rounded-full px-4 py-2 text-sm font-bold ${selected === filter.value ? "bg-[var(--navy)] text-white" : "border border-[var(--border)] bg-white text-[var(--ink-muted)] hover:border-[var(--copper)]"}`}>{filter.label}</Link>; })}</div>
        <form className="relative w-full xl:max-w-sm" action="/admin/tickets"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ink-muted)]" size={17} /><input type="search" name="q" defaultValue={search} className="field pl-10 pr-20" placeholder="Search tickets" aria-label="Search tickets" />{selected !== "ALL" && <input type="hidden" name="status" value={selected} />}<button className="absolute right-1.5 top-1.5 min-h-9 rounded-[3px] bg-[var(--navy)] px-3 text-xs font-bold text-white">Search</button></form>
      </div>
